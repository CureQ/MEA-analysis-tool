...
def define_styles():
    ...
    bigbuttonstyle.configure("bigbutton.TButton", font=(font, 50))
...
def toggle_theme():
    ...
    # For some reason you have to redefine the styles
    define_styles()
...
sidebarframe.grid(row=0, column=1, rowspan=10, sticky='nesw')
...
btn_parameter=ttk.Button(master=main_frame, text="Set Parameters", compound=LEFT, style="bigbutton.TButton", command=go_to_parameterframe)
btn_parameter.grid(row=0, column=0, padx=10, pady=10, sticky='nsew')
...
parameter_to_main.grid(row=3, column=0, padx=10, pady=(10,50), sticky='nsew')
...
default_parameters.grid(row=3, column=1, padx=10, pady=(10,50), sticky='nsew')
...
default_parameters.grid(row=3, column=2, padx=10, pady=(10,50), sticky='nsew')
    ...
    if multiprocessingvar.get():
        try:
            analyse_well(fileadress=filename, wells=wells, hertz=hertz, validation_method=validation_method, low_cutoff=low_cutoff, high_cutoff=high_cutoff, order=order, spikeduration=spikeduration,
                    exit_time_s=exit_time_s, plot_electrodes=plot_electrodes, electrode_amnt=electrode_amnt, kde_bandwidth=kde_bandwidth, smallerneighbours=smallerneighbours, minspikes_burst=minspikes_burst,
                    maxISI_outliers=maxISI_outliers, default_threshold=default_threshold, heightexception=heightexception, max_boxheight=max_boxheight, amplitude_drop_sd=amplitude_drop_sd,
...
btn_parameter=ttk.Button(master=main_frame, text="Start Analysis", style="bigbutton.TButton", command=go_to_analysisframe)
btn_parameter.grid(row=1, column=0, padx=10, pady=10, sticky='nesw')
...
results_btn=ttk.Button(master=main_frame, text="View results", style="bigbutton.TButton", command=go_to_resultfileframe)
results_btn.grid(row=2, column=0, padx=10, pady=10, sticky='nesw')
...
def view_results_func():
    ...
    if os.path.exists(f"{resultsfolder}/burst_values") and os.path.exists(f"{resultsfolder}/spike_values") and os.path.exists(f"{resultsfolder}/network_data"):
        try:
            ...
            progressinfo.grid(row=0, column=0, pady=10, padx=20)
            raw_data=openHDF5_SCA1(filename)
        except:
